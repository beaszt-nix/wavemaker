...
newState ht wt = return DT {height = ht, width = wt, samples = V.empty}
...
getSamples ds = do
  window <-
    createWindow
      ...
      defaultWindow
        { windowInitialSize =
            V2 (fromIntegral $ width ds) (fromIntegral $ height ds)
        , windowHighDPI = True
        }
...
bezierPoint t = do
  controlPoints <-
    Control.Monad.Trans.State.gets
      (V.toList . V.map (fromIntegral <$>) . samples)
  let res = round <$> deCasteljau t controlPoints
  return res

getPosition :: Event -> Maybe (Point V2 Int32)
getPosition (Event _ (MouseButtonEvent (MouseButtonEventData _ Pressed _ ButtonLeft _ pos))) =
  return pos
...
renderLoop renderer = do
  ...
  controlPoints <-
    map (fromIntegral <$>) <$>
    Control.Monad.Trans.State.gets (V.toList . samples)
  ...
  drawLine
    renderer
    (P (V2 0 (div (fromIntegral ht) 4)))
    (P (V2 (fromIntegral wt) (div (fromIntegral ht) 4)))
  drawLine
    renderer
    (P (V2 0 (div (3 * fromIntegral ht) 4)))
    (P (V2 (fromIntegral wt) (div (3 * fromIntegral ht) 4)))
  rendererDrawColor renderer $= V4 0 255 0 255
  drawLine
    renderer
    (P (V2 0 (div (fromIntegral ht) 2)))
    (P (V2 (fromIntegral wt) (div (fromIntegral ht) 2)))
  rendererDrawColor renderer $= V4 0 255 0 255
  ...
  where
    ...
    eventProcess r (Event _ (KeyboardEvent (KeyboardEventData _ Pressed _ (Keysym _ KeycodeQ _)))) =
      return ()
    ...
    eventProcess r e = do
      case getPosition e of
        Just pos -> do
          Control.Monad.Trans.State.modify
            (\x -> x {samples = V.snoc (samples x) pos})
        Nothing -> return ()
      renderLoop r
