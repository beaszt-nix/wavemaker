...
coordinateToXY (P (V2 x y)) =
  gets (fromIntegral . height) >>= \h -> return (x, (h - 2 * y) :% h)

xyToCoordinate :: (Int32, Ratio Int32) -> DrawT (Point V2 Int32)
xyToCoordinate (x, k) =
  gets (fromIntegral . height) >>= \h ->
    return $ P (V2 x (getDecimal ((h - k * h) / 2)))
  where
...
newState ht wt = do
  ...
  return DT {height = ht, width = wt, samples = smps}
...
getSamples wt = do
  ...
  window <-
    createWindow
      ...
      defaultWindow
        { windowInitialSize =
            V2 (fromIntegral $ width state) (fromIntegral $ height state)
        , windowHighDPI = True
        }
...
renderLoop renderer = do
  ...
  array <-
    Control.Monad.Trans.State.gets samples >>= (liftIO . getAssocs) >>=
    mapM xyToCoordinate
