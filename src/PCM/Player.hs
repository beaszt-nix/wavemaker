...
defaultPlayerState =
  let sample =
        genWave
          (\f n -> sin (2 * pi * fromIntegral n * fromIntegral f / 44100))
          44100
   in PlayerState
        { pitchStandard = 440
        , samples = sample
...
genWave f sr =
  runSTUArray $ do
    ...
    forM_ (range bounds) $ \x -> do writeArray arr x $ f (fromIntegral x) 1
    return arr
...
note ps (beat, semi, rest) =
  ...
  where
    ...
    noteMul semi n =
      mod (round (semitone ps semi) * n) (fromIntegral (sampleRate ps))
